...
async def run_paper_fulltext(run_id: str, paper_id: str) -> dict:
    ...
    paper simply isn't open-access in S2ORC, so the UI can distinguish
    "no full text" from "no such paper".
    """
    rs = manager.get(run_id)
    if not rs:
        raise HTTPException(status_code=404, detail="run not found")
