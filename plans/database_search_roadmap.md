...
- 2026-04-09 04:42 — completed PD-01 ✅ (new ReinforceGraph step in src/citeclaw/steps/reinforce_graph.py; hydrates rejected papers BEFORE building the citation graph so PageRank actually distinguishes them — without hydration they'd all sit at the teleport baseline; placeholder fallback when fetch_metadata fails; percentile_floor + top_n cap; restored papers get source="reinforced" and a reinforcement_log entry; 12 new tests covering constructor validation, headline rescue, screener-still-rejects, short-circuits, floor mechanics, registry; full suite 688/6 zero regressions — **Phase D started**)
- 2026-04-09 04:20 — completed PC-09 ✅ (README.md gained 4 ExpandBy* rows + Expansion family paragraph + module-layout updates; CLAUDE.md (gitignored, not staged) gained all 6 step rows including pending PD-01/PD-02 markers, an Expansion family section with thesis line, and search_model + title-only seed entries in the YAML example; verify both halves green — `grep -q "ExpandBySearch" README.md` ✓ and full suite 676/6 zero regressions — **Phase C DONE**)
- 2026-04-09 04:09 — completed PC-08 ✅ (new tests/test_expand_family_e2e.py — 5 tests covering the full ResolveSeeds→LoadSeeds→ExpandForward→Rerank→ExpandBySearch→ExpandBySemantics→ExpandByAuthor→Finalize chain; in-test _BudgetAwareFakeS2 bumps record_s2 per surface and falls back to default recs for unpredictable rerank-anchored anchor sets; corpus designs REC-1/REC-2 with A2_HIGH/A3_MID authors so ExpandByAuthor finds new candidates downstream; CITECLAW_NO_DASHBOARD=1 keeps the runner from building a TTY Dashboard; 5/5 green; full suite 676/6 zero regressions)
...
- 2026-04-09 02:51 — completed PC-03 ✅ (`ExpandByAuthor` step in src/citeclaw/steps/expand_by_author.py; reuses existing `build_author_graph` for degree_in_collab_graph metric — no author_graph.py refactor needed; constructor validates author_metric early; 8 inline smoke tests covering all 3 metrics, validation, end-to-end h_index ranking, idempotency, empty signal, citation_count alternate ranking; full suite 629/6 zero regressions)
- 2026-04-09 02:35 — completed PC-02 ✅ (`ExpandBySemantics` step in src/citeclaw/steps/expand_by_semantics.py; mirrors PC-01's structure stripped of the agent loop — one `fetch_recommendations` call does the SPECTER2 kNN; fingerprint excludes ctx.rejected so re-runs are clean; extra short-circuit reasons no_anchors + fetch_failed; 7 inline smoke tests green; full suite 629/6 zero regressions)
