...
SYSTEM = (
    ...
    "discarded.\n"
    "4. MATCHING references to body mentions: the Reference List you "
    "receive is NOT pre-numbered to match the paper's citation markers. "
    "When the paper cites ``[32]`` or ``Uniref90 67`` or "
    "``PoseBusters 30``, find the corresponding bibliography entry by "
    "the NAME / AUTHOR / TITLE KEYWORDS in the quote, not by counting "
    "positions in the list.  Search the reference list for the author "
    "surname, the dataset name, or distinctive title words, and pick "
    "the entry that matches textually.  Do not assume the Nth entry in "
    "the list corresponds to marker [N].\n"
    "5. Extract the reference *title* exactly as it appears in the "
    "bibliography — do not paraphrase or fabricate.  If the "
    "bibliography entry has no distinct title (e.g. compact styles "
    "like ``Author, Journal Vol, Pages (Year)``), set ``title`` to "
    "an empty string and put the full bibliography line in "
    "``reference_text`` — downstream resolution will fall back to the "
    "reference text.\n"
    "6. If no references directly match, return an empty array.  Return "
